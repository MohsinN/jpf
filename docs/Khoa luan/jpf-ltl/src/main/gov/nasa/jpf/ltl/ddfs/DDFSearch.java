...
public class DDFSearch extends Search {
  ...
  protected HashMap<Integer, BitSet>
    seenByDfs1 = new HashMap<Integer, BitSet> (),
  ...
  protected LinkedList<BuchiCG<String>>
    buchiStates = new LinkedList<BuchiCG<String>> ();
...
//  } catch (Throwable t) {t.printStackTrace (); throw new RuntimeException ();}
  ...
  protected Node<String> currentNode () {
    return buchiStates.size () > 0 ?
        buchiStates.peek ().node : null;
  }
  
  protected BuchiCG<String> currentBuchiCG () {
    return buchiStates.size () > 0 ?
        buchiStates.peek () : null;
  }
...
//        System.err.println ("DFS2: " + vm.getStateId () + " seen before by DFS2");
...
//        System.err.println ("DFS2: " + vm.getStateId () + " new for DFS2");
      ...
      while (!forward ()) {
        if (depth == startDepth)
          return false;
//        System.err.println ("DFS2: " + vm.getStateId () + " explored, backtrack");
        backtrack ();
      }
    }
  }
...
//      System.err.println ("â†’ " + vm.getStateId () + ", " +
...
//          "; PC = " + vm.getCurrentThread ().getPC () + "; " + firstFewCGs ());
  ...
  void setSpec (Graph<String> g, String ltl, String location) {
	assert spec == null :
		"attempted to set Buchi spec but it was already set";
	setSpec = g;
	specText = ltl;
	specSource = location;
  }
