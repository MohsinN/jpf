...
public class Dvm{
	...
	public void insertCoin(int coin1, int coin2, int coin3){
	  /* add money to coinBox, notice that coinBox can return money on user request */
		if (status > 0){
			if (coin1 > 0) {
				...
				if (coin2 > 0) {
					...
					if (coin3 > 0){
						...
						else coinBox.rejectMoney();
					}
					else if (coin3 == -1) coinBox.returnAllCoin();
				}
				else if (coin2 == -1) coinBox.returnAllCoin();
			}
			else if (coin1 == -1) coinBox.returnAllCoin();
		}
	}
