...
public class LTLSymbolicConstraintsGeneral extends SymbolicConstraintsGeneral {
	...
	Object getExpression(IntegerExpression eRef) {
		...
		if (eRef instanceof SymbolicInteger) {
			...
			if (!eRefName.contains("e")) {
				...
				if (dpvar == null) {
					// System.out.println("Make BoundIntVar " +
					// ((SymbolicInteger)eRef).getName());
			...
			case PLUS:
				if (e_leftRef instanceof IntegerConstant
					...
					throw new RuntimeException(
					    "## Error: this is not a symbolic expression"); // TODO: fix
	 ...
	 * 
	 * @param nc1
	 *          numeric constraint of the first state
	 * @param nc2
	 *          numeric constraint of the second state
	 * 
	 */
	public static boolean isConstraintSubsumed(NumericConstraint nc1,
 ...
 /**
  * return cvc3 constraint from path condition 
  */
	public Object getConstraint(PathCondition constraint, int varcount) {
		...
		while (cRef != null) {
			...
			if (cRef instanceof RealConstraint)
				constraintResult = createDPRealConstraint((RealConstraint) cRef);// create
																																				 // choco
																																				 // real
																																				 // constraint
			else if (cRef instanceof LinearIntegerConstraint)
				constraintResult = createDPLinearIntegerConstraint((LinearIntegerConstraint) cRef);// create
																																													 // choco
																																													 // linear
																																													 // integer
																																													 // constraint
				...
				throw new RuntimeException(
				    "## Error: Non Linear Integer Constraint not handled " + cRef);
