...
/**
 * This class represents a linear temporal property. This uses
 * {@link gov.nasa.ltl.trans.LTL2Buchi LTL2Buchi} to translate the LTL formula
 * to a <i>Buchi automaton</i> for infinite traces model checking. For checking
 * the finite execution traces, this uses
 * {@link gov.nasa.jpf.ltl.finite.trans.Translator Translator} to translate the
 * LTL formula to a finite state automaton. The translated automata is visualized using Jung.
 * 
...
public class LTLProperty {
  ...
  /**
   * This panel is used to display the atoms in detail. In <i>Buchi
   * automata</i>, only the short form of atoms such as p0, p1,... is
   * displayed. 
   */
  ...
  /**
   * Tries to create a new instance of this class by translating the LTL formula
   * into an automaton.
   * 
   * @param ltlSpec
   *          The LTL formula represents the temporal property.
   * @param isInfinite
   *          {@code true} if checking the infinite execution traces, {@code
   *          false} otherwise.
   * @param file
   *          {@code true} if the LTL spec. is stored in *.ltl file
   */
  public LTLProperty(String ltlSpec, boolean isInfinite, boolean file, String annotatedClassName) {
    ltlFormula = ltlSpec;
    fields = new Vector<String>();
    atoms = new Hashtable<String, String>();
    initProperty(isInfinite, file, annotatedClassName);
  }

  /**
   * Converts the Buchi translated by {@link gov.nasa.ltl.trans.LTL2Buchi
   * LTL2Buchi} to an automaton which can be used to check by {@link LTLListener}.
   * 
   * @param buchi
   *          the Buchi translated by {@link gov.nasa.ltl.trans.LTL2Buchi
   *          LTL2Buchi}
   * @return the automaton can be checked by {@link LTLListener}
   */
  ...
  private void initProperty(boolean isInfinite, boolean file, String annotatedClassName) {
    ...
    LTLSpecParser parser = new LTLSpecParser(tokens, annotatedClassName);
    ...
    try {
      ...
      finiteAutomaton = Translator.translate(formula);
  ...
  /**
   * Visualizes the translated automata using Jung. The accepting states are the
   * red vertexes.
   */
