...
import gov.nasa.jpf.symbc.numeric.LTLSymbolicConstraintsGeneral;
import gov.nasa.jpf.symbc.numeric.NumericConstraint;
 ...
 * canonicalization/GC while serializing. Despite the apparent extra work, this
 * one can be faster than its superclass because it does not consider objects
 * that become unreachable, such as class name strings and other constants.
 * 
 * @author peterd
 */
public class SymbolicFilteringSerializer extends SimpleFilteringSerializer {
  ...
  protected JVM vm;
  LTLSymbolicConstraintsGeneral ltlConstrains = new LTLSymbolicConstraintsGeneral();
  ...
  protected boolean isEquivalent(Object c1, Object c2) {
    return LTLSymbolicConstraintsGeneral.isConstraintSubsumed(
        (NumericConstraint) c1, (NumericConstraint) c2)
        && LTLSymbolicConstraintsGeneral.isConstraintSubsumed(
            (NumericConstraint) c2, (NumericConstraint) c1);
  }
...
protected int pathConditionIndex(PathCondition pc) {
  ...
  protected int[] computeStoringData() {
    ...
    buf.add(pathConditionIndex(PathCondition.getPC(vm)));
