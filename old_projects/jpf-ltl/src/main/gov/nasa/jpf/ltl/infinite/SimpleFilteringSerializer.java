 ...
 * canonicalization/GC, so objects that become unreachable after filtering are
...
public class SimpleFilteringSerializer extends AbstractSerializer {
  ...
  public void attach(JVM jvm) {
    super.attach(jvm);
    filter = jvm.getConfig().getInstance("filter.class",
  ...
  // not implemented
  /*
   * FinalBitSet getLocalFilter(MethodInfo mi) { int mid = mi.getGlobalId();
   * FinalBitSet v = localCache.get(mid); if (v == null) { BitArray a =
   * filter.getFrameLocalInclusion(mi); a.invert(); // included => filtered v =
   * FinalBitSet.create(a); if (v == null) throw new
   * IllegalStateException("Null BitSet returned."); localCache.set(mid, v); }
   * return v; }
   */
