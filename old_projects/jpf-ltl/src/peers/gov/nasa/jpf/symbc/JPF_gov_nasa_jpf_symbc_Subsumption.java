...
public class JPF_gov_nasa_jpf_symbc_Subsumption {
  ...
  static int varcount;
  // a contraint is the union of the Path Condition and the valuation ( see
  // S.Anand et al. paper: " Symbolic execution with subsumption checking")
  ...
  // store an abstracted state of heap
  static String abstractState = new String();
  // store a detail state of heap ( linearization )
  static String detailState = new String();

  /**
   * @return path condition of the current branch
   */
  ...
  // set to keep track of objects already visited; avoids revisiting when travel
  // the second time ( for debug )
  private static HashSet<Integer> discovered2;
  private static HashSet<ClassInfo> discoveredClasses; // to keep track of

  // static fields

  // goes through heap rooted in objvRef in DFS order and prints the symbolic
  // heap
  // does not print the static fields

  public static String getSymbolicRef(MJIEnv env, int objRef, int objvRef,
      int msgRef) {
  ...
  /**
   * Assumes rooted heap. A rooted heap is a pair <r, h> of a root object r and
   * a heap h such that all objects in h are reachable from r
   * 
   * Performs a DFS over objects reachable from the root, recursively.
   * 
   * Note: In DFS, discovery and finish time of nodes have parenthesis
   * structure.
   * 
   * 
   */
  private static String traverseRootedHeapAndGetSequence(MJIEnv env, int n) {
    ...
    if (n == -1) { // vertex v is null
      ...
      if (!discovered.contains(new Integer(n))) { // vertex v just discovered
        ...
        // All undiscovered successors of v are discovered. We are finished with
        // v.
  ...
  /**
   * Travel the heap and get the abstracted state of all objects.
   * 
   */
  private static void travelHeap(MJIEnv env) {
    ...
    while (iterator.hasNext()) {
      ...
      if (!discovered.contains(ei.getIndex())
          && !ei.getClassInfo().getName().contains("java.")
  ...
  /**
   * store the current state for further subsumption checking
   * 
   */
  public static void storeState(MJIEnv env, int objRef) {
    ...
    if (varcount != 0) {
      NumericConstraint nc = (NumericConstraint) ltlConstraintSolver
          .getConstraint(constraint, varcount);
  ...
  /**
   * store the current state and then check if it is subsumed by the visited
   * state
   * 
   * @return {@code true} if the current state is subsumed, {@code false}
   *         otherwise
   */
   ...
   * 
   * @param nc
   *          the numeric contraint need to be checked if it is subsumed by
   *          other numeric constraints
   * @param constraints
   *          list contraints
   * @return {@code true} if the nc is subsumed by one in constraints, {@code
   *         false} otherwise
   */
  private static boolean isConstraintSubsumed(NumericConstraint nc,
      ArrayList<NumericConstraint> constraints) {
    for (int i = 0; i < constraints.size(); i++) {
      System.out.println("Compare with constraint " + i);
      NumericConstraint temp = constraints.get(i);
      System.out.println(temp);
      if (temp.getExpr() != null
          && LTLSymbolicConstraintsGeneral.isConstraintSubsumed(nc, temp)) {
