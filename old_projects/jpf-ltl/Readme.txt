...
temporal logic to describe the semantics of finite or infinite model
checking, separately.

The LTL to Buchi automata translation module used in this model checker
is a modified version of the existing LTL2Buchi tool; it is located at
http://bitbucket.org/estar/ltl2buchi  

This model checker supports symbolic and concrete mixed execution of the 
system under test (SUT).

This project's repository is http://bitbucket.org/francoraimondi/jpf-ltl
...
You must annotate your target class with the @LTLSpec or @LTLSpecFile
annotation. @LTLSpec takes a String parameter which contains the
specification formula; @LTLSpecFile takes a String parameter which is a
path to a text file containing the formula. 
...
You have to decide in advance whether the system under test will
terminate or run infinitely. In the first case, specify
  finite=true
in the .jpf file; in the second case, specify
  finite=false
  search.class=gov.nasa.jpf.ltl.ddfs.DDFSearch
  vm.storage.class=gov.nasa.jpf.jvm.FullStateSet
  vm.serializer=gov.nasa.jpf.ltl.ddfs.SymbolicFilteringSerializer